...
$packageArgs = @{
  ...
  fileType      = 'exe'
  silentArgs    = '"/U:' + ${env:ProgramFiles(x86)} + '\Stardock\Start11\Uninstall\uninstall.xml"'
...
if ($key.Count -eq 1) {
  $key | ForEach-Object {
    ...
    ${env:ProgramFiles(x86)} + "\Stardock\Start11\uninstall.exe" + " $($packageArgs['silentArgs'])" #> $null
