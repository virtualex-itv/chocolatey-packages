...
function CreateStream {
  ...
  $productBinariesUrl = "https://my.vmware.com/channel/public/api/v1.0/products/getRelatedDLGList?locale=en_US&category=desktop_end_user_computing&product=vmware_workstation_player&version=$($productVersion)&dlgType=PRODUCT_BINARY"

  $jsonProduct = Invoke-WebRequest -Uri $productBinariesUrl | ConvertFrom-Json

  $product = $jsonProduct.dlgEditionsLists.dlgList
...
function global:au_GetLatest {
  ...
  $productHeaderUrl = "https://my.vmware.com/channel/public/api/v1.0/products/getProductHeader?locale=en_US&category=desktop_end_user_computing&product=vmware_workstation_player&version=$($productVersion)"
