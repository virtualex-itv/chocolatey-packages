...
$packageArgs = @{
  ...
  fileType      = 'exe'
  silentArgs    = '"/U:' + ${env:ProgramFiles(x86)} + '\Stardock\Start8\Uninstall\uninstall.xml"'
...
if ($key.Count -eq 1) {
  $key | ForEach-Object {
    ...
    ${env:ProgramFiles(x86)} + "\Stardock\Start8\uninstall.exe" + " $($packageArgs['silentArgs'])" #> $null
