...
# Fixed by: virtualex-itv - filter empty strings after split to prevent Substring error
...
foreach ($commit in $all_commits.Matches.Value) {
    ...
    # FIX: Filter out empty strings after split to prevent Substring error
    $packages_md = $packages -split ' ' | Where-Object { $_.Length -gt 0 } | ForEach-Object {
